% Generate simulation specifications for tuning MKF_SF
...
for i_comb = 1:n_combs
    ...
    nf = n_di_idx{i_comb};  % fusion horizon
    ...
    if isequal([MKF_SF1.f, MKF_SF1.f, MKF_SF1.m, MKF_SF1.d], ...
            [20 1 2])
        disp("stop")
    end

    if MKF_SF1.nh > nh_max
        fprintf("Skipping simulation due to nh: %d > %d\n", ...
            MKF_SF1.nh, nh_max)
        continue
    elseif MKF_SF1.beta < beta_min
        fprintf("Skipping simulation due to beta: %.2f < %.2f\n", ...
            MKF_SF1.beta, beta_min)
        continue
    end
